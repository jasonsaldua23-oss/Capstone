...
export async function PATCH(
  ...
  try {
    ...
    const completedDropPoints = tripDropPoints.filter((s) => s.status === 'COMPLETED').length
    const allDone = tripDropPoints.every((s) => ['COMPLETED', 'FAILED', 'SKIPPED'].includes(s.status))
