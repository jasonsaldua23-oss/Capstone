    ...
    >
      ...
      <Card className="w-full max-w-[420px] rounded-[24px] border border-[#dce3ec] bg-white/95 shadow-[0_16px_42px_rgba(15,23,42,0.14)] backdrop-blur-sm">
        <CardHeader className="space-y-2 pb-0 pt-6">
          ...
          <CardTitle className=" text-[#112b60] text-2xl text-center font-extrabold leading-tight">
            <span className="block">Ann Ann&apos;s Beverages Trading</span>
            <span className="block">Warehouse Staff</span>
          </CardTitle>
          <CardDescription className="text-[#7a89a6] text-center text-[15px]">Log in with your warehouse staff account.</CardDescription>
        </CardHeader>
        <CardContent className="px-7 pb-6">
          <form onSubmit={handleLogin} autoComplete="off" className="space-y-3">
            <div className="space-y-2">
              <Label htmlFor="warehouse-email" className="text-[#1f3566] text-sm font-semibold">Email</Label>
              <div className={`relative h-11 rounded-xl border bg-white ${loginError ? 'border-rose-300' : 'border-[#d6deea]'}`}>
                ...
                <Input id="warehouse-email" type="email" autoComplete="off" value={email} onChange={(e) => { setEmail(e.target.value); if (loginError) setLoginError('') }} placeholder="Enter email" required aria-invalid={Boolean(loginError)} className="h-full border-0 bg-transparent pl-10 text-slate-900 placeholder:text-[#9aa8bf] focus-visible:ring-0" />
              </div>
            </div>
            <div className="space-y-2">
              <Label htmlFor="warehouse-password" className="text-[#1f3566] text-sm font-semibold">Password</Label>
              <div className={`relative h-11 rounded-xl border bg-white ${loginError ? 'border-rose-300' : 'border-[#d6deea]'}`}>
                <Lock className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[#8a99b3]" />
                <Input id="warehouse-password" type={showPassword ? 'text' : 'password'} autoComplete="new-password" value={password} onChange={(e) => { setPassword(e.target.value); if (loginError) setLoginError('') }} placeholder="Enter password" required aria-invalid={Boolean(loginError)} className="h-full border-0 bg-transparent pl-10 pr-11 text-slate-900 placeholder:text-[#9aa8bf] focus-visible:ring-0" />
            ...
            <Link
              href={forgotPasswordHref('warehouse', email)}
