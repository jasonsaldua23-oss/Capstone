...
const buildProductSku = (name: unknown, unit: unknown, size: unknown, suffix: unknown) => {
  const part = (value: unknown, fallback: string, length: number) =>
    String(value || fallback).toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, length) || fallback
  return `${part(name, 'PRD', 4)}-${part(unit, 'UNT', 3)}-${part(size, 'SIZE', 4)}-${part(suffix, 'ITEM', 5)}`
}
...
export function WarehouseProductForms({ warehouse, products = [], children }: {
  ...
  const autoGeneratedSku = useMemo(() => {
    ...
    return buildProductSku(productName, productUnit, selectedProductSize, productSkuSeed)
  }, [productName, productUnit, selectedProductSize, productSkuSeed])

  const autoGeneratedEditSku = useMemo(() => {
    if (!editingItem?.product?.id) return ''
    const suffix = String(editingItem.product.id).replace(/[^a-z0-9]/gi, '').slice(-5)
    return buildProductSku(editName, editUnit, editSize, suffix)
  }, [editName, editSize, editUnit, editingItem])

  useEffect(() => {
    if (!editingItem?.product) return
    const originalSize = Array.isArray(editingItem.product.sizes)
      ? String(editingItem.product.sizes[0] || '').trim()
      : ''
    const identityChanged = normalizeProductIdentityPart(editName) !== normalizeProductIdentityPart(editingItem.product.name)
      || normalizeProductIdentityPart(editUnit) !== normalizeProductIdentityPart(editingItem.product.unit)
      || normalizeProductIdentityPart(editSize) !== normalizeProductIdentityPart(originalSize)
    // Fix: product identity edits must immediately show the matching generated SKU.
    setEditSku(identityChanged ? autoGeneratedEditSku : String(editingItem.product.sku || ''))
  }, [autoGeneratedEditSku, editName, editSize, editUnit, editingItem])
