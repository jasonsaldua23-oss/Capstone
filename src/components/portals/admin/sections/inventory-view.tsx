...
export function InventoryView() {
  ...
  const registerProduct = async () => {
    ...
    const hasInitialStock = productInitialStock.trim() !== ''
    const nextInitialStock = hasInitialStock ? Number(productInitialStock) : 0
    ...
    if (hasInitialStock && (!Number.isFinite(nextInitialStock) || nextInitialStock < 0)) {
      return toast.error('Invalid initial stock quantity')
    }
    ...
    try {
      ...
      const response = await fetch('/api/products', {
        ...
        body: JSON.stringify({
          ...
          initialQuantity: hasInitialStock ? Math.floor(nextInitialStock) : undefined,
        }),
      })
      const payload = await response.json().catch(() => ({}))
      if (!response.ok || payload?.success === false) {
                  ...
                  {filteredInventory.map((item) => {
                    ...
                    return (
                      <tr key={item.id} className="border-b last:border-0 hover:bg-gray-50">
                        ...
                        <td className="p-4 font-semibold text-gray-900">{getThreshold(item)}</td>
      ...
      >
        <DialogContent className="max-w-5xl w-full">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-1">
              <label className="text-sm font-medium text-gray-700">Initial Stock Quantity (Optional)</label>
              <Input
                type="number"
                step="1"
