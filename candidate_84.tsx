...
import {
  ...
  LocateFixed, Trophy, RotateCcw, Search, Volume2, VolumeX
} from 'lucide-react'
...
export function TripDetailView({
  ...
  const MAX_MAP_ACCEPTABLE_ACCURACY_METERS = 5000 // Relaxed for laptop/IP-based development testing
  ...
  const isReasonableGps = (lat: number, lng: number, accuracy?: number | null) => {
    // We enforce bounds so that laptop IP-based locations in other countries don't break the local route map.
    return isWithinNegrosBounds(lat, lng)
  }

  const isFreshRecordedAt = (value: unknown, maxAgeMs: number) => {
    // Bypass age checking during development to ensure marker renders
    return true
  }
  ...
  // Move to the next maneuver as the live vehicle position reaches it.
  useEffect(() => {
    if (!effectiveDriverLocation || routeSteps.length === 0 || currentStepIndex >= routeSteps.length - 1) return

    const nextStep = routeSteps[currentStepIndex + 1]
    const distanceToNextStepKm = haversineKm(effectiveDriverLocation, {
      lat: nextStep.maneuver.location[1],
      lng: nextStep.maneuver.location[0],
    })

    if (distanceToNextStepKm <= 0.05) {
      setCurrentStepIndex((previous) => Math.min(previous + 1, routeSteps.length - 1))
    }
  }, [effectiveDriverLocation, routeSteps, currentStepIndex])
  ...
  const routeWaypointsKey = routeWaypoints
    ...
    .join('|')
  const navigationRouteWaypoints = upcomingRouteWaypoints.length > 1 ? upcomingRouteWaypoints : routeWaypoints
  // Quantize only the moving origin (~100 m) to reroute without calling OSRM on every GPS update.
  const navigationWaypointsKey = navigationRouteWaypoints
    .map((point, index) => `${point.lat.toFixed(index === 0 ? 3 : 6)},${point.lng.toFixed(index === 0 ? 3 : 6)}`)
    .join('|')

  useEffect(() => {
    const requestWaypoints = navigationWaypointsKey
      .split('|')
      .map((coordinate) => {
    ...
    const uniqueWaypoints = requestWaypoints.filter((point, index, list) => {
  ...
  const completedRoutePoints = (() => {
    const points = completedRouteWaypoints.map(
      (point) => [point.lat, point.lng] as [number, number]
    )
    // Prepend warehouse start if available and no driver location yet
    if (warehouseRouteStart && !driverLocationMarker && pendingDropPoints.length > 0) {
      return [[warehouseRouteStart.lat, warehouseRouteStart.lng] as [number, number], ...points]
    }
    return points
  })()
  const upcomingRoutePoints = (() => {
    const points = upcomingRouteWaypoints.map(
      (point) => [point.lat, point.lng] as [number, number]
    )
    // Prepend warehouse start if available so the route shows from warehouse to first stop
    if (warehouseRouteStart && points.length > 0) {
      const firstPoint = points[0]
      const isAlreadyWarehouseStart =
        Math.abs(firstPoint[0] - warehouseRouteStart.lat) < 0.0001 &&
        Math.abs(firstPoint[1] - warehouseRouteStart.lng) < 0.0001
      if (!isAlreadyWarehouseStart) {
        return [[warehouseRouteStart.lat, warehouseRouteStart.lng] as [number, number], ...points]
      }
    }
    return points
          ...
          <div className="relative overflow-hidden md:hidden">
            <div className="relative h-[calc(100dvh-12rem)] min-h-[540px] w-full overflow-hidden bg-[#dff0ea]">
              ...
              <button
                ...
                className={`absolute left-4 z-30 flex h-9 w-9 items-center justify-center rounded-full border border-white/80 bg-white/92 text-slate-900 shadow-[0_8px_18px_rgba(15,23,42,0.12)] backdrop-blur transition hover:bg-white ${trip.status === 'IN_PROGRESS' && routeSteps.length > 0 ? 'top-[5.5rem]' : 'top-4'}`}
              >
                <ChevronLeft className="h-5 w-5" />
              </button>
              {/* Removed Route Map and Exact labels */}
            ...
            <button
              ...
              className={`absolute bottom-[calc(env(safe-area-inset-bottom)+9.4rem)] right-4 z-40 flex h-10 w-10 items-center justify-center rounded-full border shadow-[0_8px_18px_rgba(15,23,42,0.12)] backdrop-blur transition-all ${is3DPerspective ? 'bg-sky-600 border-sky-700 text-white' : 'bg-white/92 border-white/80 text-slate-900'}`}
            >
            ...
            <button
              ...
              className={`absolute bottom-[calc(env(safe-area-inset-bottom)+12.4rem)] right-4 z-40 flex h-10 w-10 items-center justify-center rounded-full border shadow-[0_8px_18px_rgba(15,23,42,0.12)] backdrop-blur transition-all ${voiceGuidanceEnabled ? 'bg-emerald-600 border-emerald-700 text-white' : 'bg-white/92 border-white/80 text-slate-900'}`}
            >
